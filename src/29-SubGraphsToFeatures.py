...
def mnist_tutorial_cw(train_start=0, train_end=60000, test_start=0,
    ...
    for l1 in range(len(FamilyNames)):
        ...
        MisclassificationFrom = [0,0,0,0]
        TMisclassification = [0,0,0,0]
        NodesAdded = [0,0,0,0]
        UNNodesAdded = [0,0,0,0]
        All = 0
                    ...
                    for compSize in range(3):
                        ...
                        for files1 in os.listdir(directory1):
                            ...
                            for i in range(ToRepeat):
                                orgGraph  = nx.drawing.nx_agraph.read_dot(loc1)
                                orgGraph = orgGraph.to_directed()
                                gMade = nx.DiGraph()
                                Nodes = list(orgGraph.nodes())
                                Edges = list(orgGraph.edges())
                                for j in range(len(Nodes)):
                                    gMade.add_node((i*100)+1 + int(Nodes[j]))
                                for j in range(len(Edges)):
                                    gMade.add_edge((i*100)+1 + int(Edges[j][0]),(i*100)+1 + int(Edges[j][1]))
                                # gMade = gMade.to_directed()
                                g2.append(gMade)
                            g1 = nx.drawing.nx_agraph.read_dot(loc)
                            g1 = nx.DiGraph(g1)
                            g0 = g1
                            for i in range(ToRepeat):
                                g0 = nx.compose(g0,g2[i])
                            g = g0
                            n1 = (g1.nodes())
                            n2 = []
                            for i in range(ToRepeat):
                                n2.append(g2[i].nodes())
                            nodesToAdd = len(n2[0])*ToRepeat
                            xs = []
                            n1 = np.asarray(n1)
                            n2 = np.asarray(n2)
                            ...
                            for i in range(ToRepeat):
                                g.add_edge(hex(nEntry_Label),n2[i][0])
                            ...
                            for i in range(ToRepeat):
                                g.add_edge(n2[i][len(n2)-1],hex(nExit_Label))

                            # write_dot(g,"/home/ahmed/Documents/Projects/IoT_Attack_Journal/Code/SubGraphs/Comp.dot")
                            # exit()
                            #### Start from here ####
                            ...
                            if predicting != y[0] :
                                if alreadyMisclass == 0:
                                    ...
                                    MisclassificationFrom[l]+=1
                                    alreadyMisclass = 1
                                    UNNodesAdded[l] += nodesToAdd
                                    # print("Misclassified")
                                if predicting == l:
                                    if alreadyTMisclass != 1 :
                                        TMisclassification[l] += 1
                                        NodesAdded[l] += nodesToAdd
                                        alreadyTMisclass = 1
                                        # print("Success")
                                        break
                        if alreadyTMisclass == 1:
        ...
        print("Misclassification ORG",MisclassificationFrom)
        print("Targeted Misclassification",TMisclassification)
        for i in range(4):
            if TMisclassification[i]!=0:
                NodesAdded[i] = NodesAdded[i]/TMisclassification[i]
            if Misclassification[i]!=0:
                UNNodesAdded[i] = UNNodesAdded[i]/Misclassification[i]
        print("Targeted Nodes Added",NodesAdded)
        print("Untargeted Nodes Added",UNNodesAdded)
